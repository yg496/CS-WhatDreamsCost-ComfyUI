...
这是基于 [WhatDreamsCost/WhatDreamsCost-ComfyUI](https://github.com/WhatDreamsCost/WhatDreamsCost-ComfyUI) 的 CS 改造版本，用于在 LTX Director 2.0 基础上接入 2x2 四宫格、3x2 六宫格、3x3 九宫格自动拆分分镜。

本版本已做独立命名隔离，节点类型、前端扩展名、后端接口路径和上传子目录均使用 `CS` 前缀，方便在 RunningHub 或本地环境中与原作者版本同时存在，避免覆盖原版节点。
...
```bash
git clone https://github.com/yg496/CS-WhatDreamsCost-ComfyUI
```

原作者项目与 2.0 功能说明仍保留在下方，版权与许可证遵循原项目授权。
