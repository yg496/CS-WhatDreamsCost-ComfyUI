...
def _snap_latent_to_downscale(latent_image, noise_mask, downscale_factor, method):
    ...
    log.warning(
        "[CSLTXDirectorGuide] Auto-snapping latent grid from %sx%s to %sx%s for IC-LoRA downscale factor %s.",
...
def _load_lora_model_only(model, ic_lora_name, strength_model):
    ...
    try:
        ...
        log.warning(
            "[CSLTXDirectorGuide] Could not read reference_downscale_factor from %s, using 1.0.",
            ic_lora_name,
        )
...
def _load_motion_video_frames(video_file, trim_start_frames, length_frames, director_fps, resample_mode="nearest"):
    ...
    log.info(f"[CSLTXDirectorGuide] Loading video frames from {path}. start_s: {start_s:.3f}, dur_s: {dur_s:.3f}, end_s: {end_s}")
    ...
    if start_s > 0:
        try:
            ...
            log.info(f"[CSLTXDirectorGuide] PyAV seeked stream to pts={seek_pts} (approx {max(0, start_s - 0.5):.3f}s)")
        except Exception as seek_err:
            log.warning(f"[CSLTXDirectorGuide] Seek failed: {seek_err}, decoding from beginning.")
...
class CSLTXDirectorGuide:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                "positive": ("CONDITIONING", {"display_name": "正向条件", "tooltip": "需要加入引导关键帧信息的正向条件。"}),
                "negative": ("CONDITIONING", {"display_name": "负向条件", "tooltip": "需要加入引导关键帧信息的负向条件。"}),
                "vae": ("VAE", {"display_name": "视频 VAE", "tooltip": "用于编码引导图像的视频 VAE。"}),
                "latent": ("LATENT", {"display_name": "视频潜空间", "tooltip": "引导会被插入到这个视频潜空间中。"}),
                "guide_data": ("GUIDE_DATA", {"display_name": "引导数据", "tooltip": "由 LTX 导演台输出的图像引导数据。"}),
            },
            "optional": {
                "motion_guide_data": ("MOTION_GUIDE_DATA", {"display_name": "运动引导数据", "tooltip": "连接导演台输出的运动视频引导数据，用于 IC-LoRA 视频引导。"}),
                "model": ("MODEL", {"display_name": "模型", "tooltip": "使用 IC-LoRA 运动引导时连接模型。"}),
                "ic_lora_name": (["None"] + loras, {"default": "None", "display_name": "IC-LoRA 名称", "tooltip": "选择用于运动引导的 IC-LoRA 模型。"}),
                "ic_lora_strength": ("FLOAT", {"default": 1.0, "min": -100.0, "max": 100.0, "step": 0.01, "display_name": "IC-LoRA 强度"}),
                "scale_by": ("FLOAT", {"default": 1.0, "min": 0.01, "max": 8.0, "step": 0.01, "display_name": "缩放倍率", "tooltip": "按该倍率缩放潜空间。"}),
                "upscale_method": (["nearest-exact", "bilinear", "area", "bicubic", "bislerp"], {"default": "bicubic", "display_name": "缩放算法", "tooltip": "用于放大或缩小潜空间的方法。"}),
                "image_attention_strength": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.01, "display_name": "图像注意力强度"}),
                "crop": (["disabled", "center"], {"default": "center", "display_name": "裁剪方式"}),
                "auto_snap_ic_grid": ("BOOLEAN", {"default": True, "display_name": "自动对齐 IC 网格"}),
                "use_tiled_encode": ("BOOLEAN", {"default": False, "display_name": "分块编码"}),
                "tile_size": ("INT", {"default": 256, "min": 64, "max": 512, "step": 32, "display_name": "分块大小"}),
                "tile_overlap": ("INT", {"default": 64, "min": 16, "max": 256, "step": 16, "display_name": "分块重叠"}),
                "retake_mode": ("BOOLEAN", {"default": False, "display_name": "强制 Retake 模式", "tooltip": "开启后强制使用 Retake 模式；关闭时仍会根据导演台时间线自动检测。"}),
            }
        }

    RETURN_TYPES = ("CONDITIONING", "CONDITIONING", "LATENT", "MODEL", "FLOAT")
    RETURN_NAMES = ("正向条件", "负向条件", "视频潜空间", "模型", "潜空间缩放倍率")
    FUNCTION = "execute"
    ...
    def execute(cls, positive, negative, vae, latent, guide_data, motion_guide_data=None, model=None, ic_lora_name="None", ic_lora_strength=1.0, scale_by=1.0, upscale_method="bicubic", image_attention_strength=1.0, crop="center", auto_snap_ic_grid=True, use_tiled_encode=False, tile_size=256, tile_overlap=64, retake_mode=False):
        ...
        print(f"[CSLTXDirectorGuide] execute started. motion_segments: {len(motion_segments)}, image_guides: {image_guides_count}, ic_lora_name: {ic_lora_name}, model connected: {model is not None}, retake_mode: {retake_mode}")
        ...
        # -----------------------------------------------------------------------
        if is_retake_active:
            print(f"[CSLTXDirectorGuide] Retake Mode active. Preserving base latent, masking selected regions. is_empty_latent: {is_empty_latent}")
            ...
            if not is_empty_latent and l_start == 0 and l_end >= latent_length:
                ...
                print("[CSLTXDirectorGuide] Stage 2: Retake region covers the entire generated range. Skipping base video loading and VAE encoding.")
            ...
            if video_file and need_base_video:
                try:
                    print(f"[CSLTXDirectorGuide] Loading and encoding base video file: {video_file} starting at frame {start_frame} for length {ltxv_length} at resolution {target_width}x{target_height}")
                    ...
                    if is_empty_latent:
                        ...
                        print(f"[CSLTXDirectorGuide] Stage 2: Copying high-resolution base latent for preserved regions (0-{l_start} and {l_end}-{paste_len})")
                        if l_start > 0:
                            latent_image[:, :, :l_start] = base_latent[:, :, :l_start]
                        if l_end < paste_len:
                            latent_image[:, :, l_end:paste_len] = base_latent[:, :, l_end:paste_len]
                except Exception as e:
                    print(f"[CSLTXDirectorGuide] Failed to load/encode base video: {e}. Falling back to input latent.")
            ...
            print(f"[CSLTXDirectorGuide] noise_mask slice: {noise_mask[0, 0, :, 0, 0].tolist()}")
        ...
        if len(images) > 0 or len(segments) > 0:
            print(f"[CSLTXDirectorGuide] Using Appended Keyframe Guidance. is_lora_active: {is_lora_active}")
            ...
            print("[CSLTXDirectorGuide] No timeline guides present. Passing through.")
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "latent": ("LATENT", {"display_name": "视频潜空间"}),
            }
        }
