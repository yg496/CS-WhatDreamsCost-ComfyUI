...
const ZH_DIRECTOR_INPUT_LABELS = {
  model: "模型",
  clip: "文本编码器",
  audio_vae: "音频 VAE",
  ...
  duration_frames: "总帧数",
  timeline_data: "时间线数据",
  use_custom_audio: "使用时间线音频",
  use_custom_motion: "使用运动视频引导",
  inpaint_audio: "补全音频空隙",
  local_prompts: "分段提示词",
  segment_lengths: "分段长度",
  epsilon: "分段边界强度",
  frame_rate: "帧率",
  display_mode: "时间显示方式",
  guide_strength: "引导强度",
  ...
  override_audio: "使用 IC 视频音频",
  grid_mode: "宫格模式",
  grid_images: "宫格图像",
  grid_text: "宫格分镜文本",
  grid_layout: "宫格模式选择",
  grid_aspect: "分镜比例",
  grid_border_crop: "白边裁剪强度",
  grid_auto_refresh: "宫格自动刷新",
  timeline_ui: "导演台时间线",
};

const ZH_DIRECTOR_OUTPUT_LABELS = {
  model: "模型",
  positive: "正向条件",
  video_latent: "视频潜空间",
  audio_latent: "音频潜空间",
  guide_data: "引导数据",
  motion_guide_data: "运动引导数据",
  frame_rate: "帧率",
  combined_audio: "合成音频",
};

function setLiteGraphLabel(item, label) {
...
function applyChineseDirectorLabels(node) {
  if (!node) return;
  node.title = node.title || "LTX 2.0 导演台";
  for (const input of node.inputs || []) {
    setLiteGraphLabel(input, ZH_DIRECTOR_INPUT_LABELS[input.name]);
  }
  for (const output of node.outputs || []) {
    setLiteGraphLabel(output, ZH_DIRECTOR_OUTPUT_LABELS[output.name]);
  }
  for (const widget of node.widgets || []) {
    setLiteGraphLabel(widget, ZH_DIRECTOR_INPUT_LABELS[widget.name]);
  }
}
...
function getWidgetValue(node, name, fallback = "") {
  ...
  return widget && widget.value !== undefined && widget.value !== null ? widget.value : fallback;
}
...
function loadImageNodeUrl(node) {
  ...
  const rawType = String(info.type || getWidgetValue(node, "type", "input") || "input").toLowerCase();
...
function getGridLayoutDims(value) {
  const text = String(value || "");
  const m = text.match(/(\d+)\s*x\s*(\d+)/i);
  if (m) return { cols: Math.max(1, Number(m[1])), rows: Math.max(1, Number(m[2])) };
  if (text.includes("九")) return { cols: 3, rows: 3 };
  if (text.includes("四")) return { cols: 2, rows: 2 };
  return { cols: 3, rows: 2 };
}

function isGridModeEnabled(node) {
  return !!getWidgetValue(node, "grid_mode", false);
}

function getGridSource(node) {
  const origin = getOriginNode(node, "grid_images");
  if (!origin) return null;
  const { cols, rows } = getGridLayoutDims(getWidgetValue(node, "grid_layout", "3x2 六宫格"));
  let url = "";
  ...
  if (nodeType === "LoadImage" || nodeType === "LoadImageMask" || getWidget(origin, "image") || getWidget(origin, "图像")) {
    url = loadImageNodeUrl(origin);
  }
  if (!url && origin.imgs?.[0]?.src) url = origin.imgs[0].src;
  if (!url && origin.image?.src) url = origin.image.src;
  return { cols, rows, url };
}

function gridSourceKey(source) {
  if (!source?.url) return "";
  return `${source.cols}x${source.rows}|${source.url}`;
}
...
function cleanGridPrompt(prompt) {
  const numerals = "一二三四五六七八九十";
  const marker = new RegExp(`^\\s*(?:分镜|镜头|画面|shot)\\s*[${numerals}\\d]+\\s*[:：.\\-、]\\s*`, "i");
  return String(prompt || "").replace(marker, "").replace(/\s+/g, " ").trim();
}

function parseGridPrompts(text) {
  const stripped = stripFence(text);
  if (!stripped) return [];
  try {
    let data = JSON.parse(stripped);
    if (!Array.isArray(data) && data && typeof data === "object") {
      ...
      if (!Array.isArray(data) && data && typeof data === "object") {
        const numbered = Object.entries(data)
          .map(([key, value]) => [Number(String(key).match(/\d+/)?.[0] || NaN), value])
          .filter(([idx]) => Number.isFinite(idx));
        data = numbered.length ? numbered.sort((a, b) => a[0] - b[0]).map(([, value]) => value) : Object.values(data);
      }
    }
    if (Array.isArray(data)) {
      return data.map(item => {
        if (typeof item === "string") return cleanGridPrompt(item);
        if (item && typeof item === "object") {
          for (const key of ["prompt", "description", "text", "content", "scene", "action", "动态描述", "描述", "提示词", "画面"]) {
            if (item[key]) return cleanGridPrompt(item[key]);
          }
        }
        return "";
      }).filter(Boolean);
  ...
  if (matches.length) {
    return matches.map((match, idx) => {
      ...
      return cleanGridPrompt(stripped.slice(start, end));
    }).filter(Boolean);
  }
  return stripped.split(/\n+/).map(cleanGridPrompt).filter(Boolean);
}

function evenGridLengths(totalFrames, count) {
  totalFrames = Math.max(count, Math.round(Number(totalFrames) || count));
  const base = Math.floor(totalFrames / count);
  const lengths = Array(count).fill(Math.max(1, base));
...
function imageFromUrl(url) {
  return new Promise((resolve, reject) => {
    ...
    img.src = `${url}${sep}_grid_preview=${Date.now()}`;
  });
}
...
class TimelineEditor {
  constructor(node, container, domWidget) {
    ...
    this._lastGridSourceKey = "";
    this._lastGridInputKey = "";
    this._lastGridCheck = 0;
    this._gridPreviewImage = null;
    this._gridPreviewKey = "";
    this._gridPreviewLoad = null;
    this._gridAutoRefreshTimer = null;
    ...
    this.gridModeWidget = this.node.widgets.find(w => w.name === "grid_mode");
    this.gridTextWidget = this.node.widgets.find(w => w.name === "grid_text");
    this.gridLayoutWidget = this.node.widgets.find(w => w.name === "grid_layout");
    this.gridAspectWidget = this.node.widgets.find(w => w.name === "grid_aspect");
    this.gridBorderCropWidget = this.node.widgets.find(w => w.name === "grid_border_crop");
    this.gridAutoRefreshWidget = this.node.widgets.find(w => w.name === "grid_auto_refresh");
    ...
    console.log("[CSLTXDirector debug] Constructor: timelineDataWidget value:", this.timelineDataWidget?.value);
    ...
    console.log("[CSLTXDirector debug] Constructor: parsed timeline:", JSON.stringify(this.timeline));
    ...
    const origGridModeCallback = this.gridModeWidget?.callback;
    if (this.gridModeWidget) {
      this.gridModeWidget.callback = (...args) => {
        if (origGridModeCallback) origGridModeCallback.apply(this.gridModeWidget, args);
        this.updateGridUIState();
        this.commitChanges(true);
        this.render();
      };
    }
    for (const w of [this.gridLayoutWidget, this.gridAspectWidget, this.gridBorderCropWidget]) {
      if (!w) continue;
      const orig = w.callback;
      w.callback = (...args) => {
        if (orig) orig.apply(w, args);
        this._lastGridInputKey = "";
        if (isGridModeEnabled(this.node)) this.requestGridAutoRefresh("settings");
        this.render();
      };
    }
  ...
  async _getOrExtractAudio(seg) {
    ...
    if (this._audioExtractionPromises.has(fileKey)) {
      try {
        ...
        console.warn("[CSLTXDirector] Awaiting shared server audio extract promise failed:", err);
      }
      return;
    }

    const extractionPromise = (async () => {
      const resp = await api.fetchApi(`/cs_ltx_director_get_audio?filename=${encodeURIComponent(fileKey)}`);
      if (resp.status === 200) {
    ...
    try {
      ...
      console.warn("[CSLTXDirector] Server audio check/extract failed:", err);
    } finally {
  ...
  createDOM() {
    ...
    autoGridBtn.className = "pr-btn";
    autoGridBtn.innerHTML = `${ICONS.upload} 自动填充宫格`;
    autoGridBtn.title = "根据宫格图像自动生成主轨分镜";
    autoGridBtn.addEventListener("click", () => this.autoFillFromGrid(false));
    this.autoGridBtn = autoGridBtn;

    const syncGridTextBtn = document.createElement("button");
    syncGridTextBtn.className = "pr-btn";
    syncGridTextBtn.innerHTML = `${ICONS.text} 同步宫格文本`;
    syncGridTextBtn.title = "只同步分镜文本，不改变分镜图像和时长";
    syncGridTextBtn.addEventListener("click", () => this.syncGridPromptsFromText());
    this.syncGridTextBtn = syncGridTextBtn;
    ...
    uploadMotionBtn.innerHTML = `${ICONS.motion} 添加 IC 视频`;
    ...
    uploadVideoBtn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg> 添加视频`;
    ...
    addTextBtn.innerHTML = `${ICONS.text} 添加文字`;
    ...
    actionGroup.appendChild(autoGridBtn);
    actionGroup.appendChild(syncGridTextBtn);
    ...
    deleteRetakeBtn.title = "移除 Retake 视频";
    ...
    this.segmentBoundsDisplay.textContent = "开始：- | 结束：- | 长度：-";
    ...
    inpaintToggleBtn.textContent = "补音：开";
    inpaintToggleBtn.title = "切换音频空隙补全";

    this.updateInpaintToggleStyle = (isOn) => {
      inpaintToggleBtn.textContent = isOn ? "补音：开" : "补音：关";
      if (isOn) {
    ...
    this.syncInpaintState = () => {
      ...
      if (customAudioWidget && !customAudioWidget.value) {
        ...
        inpaintToggleBtn.title = "需要开启自定义音频后才能补全音频空隙";
      } else {
    ...
    overrideAudioToggleBtn.textContent = "音频：关";
    overrideAudioToggleBtn.title = "使用 IC-LoRA 视频里的音频";

    this.updateOverrideAudioToggleStyle = (isOn) => {
      overrideAudioToggleBtn.textContent = isOn ? "音频：开" : "音频：关";
      if (isOn) {
    ...
    const updateSnapStyle = () => {
      snapBtn.title = this.isSnapping ? "关闭吸附" : "开启吸附";
      if (this.isSnapping) {
    ...
    retakeToggleBtn.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.5 2v6h-6M21.34 15.57a10 10 0 1 1-.57-8.38l5.67-5.67"/></svg> <span>Retake 模式（测试）</span>`;

    const updateRetakeStyle = () => {
      retakeToggleBtn.title = this.retakeMode ? "切换到多片段时间线" : "切换到 Retake 模式";
      if (this.retakeMode) {
    ...
    this.globalPromptInput.placeholder = "输入整段视频的全局提示词...";
    ...
    this.mainTrackLabel = createTrackLabel("主轨", "#1e1e1e", "main", this.mainTrackEnabled, () => {
    ...
    this.motionTrackLabel = createTrackLabel("IC 视频", "#1e1e1e", "motion", this.motionTrackEnabled, () => {
  ...
  syncWidgetsAndUI() {
    console.log("[CSLTXDirector debug] syncWidgetsAndUI() called.");
  ...
  updateGridUIState() {
    const enabled = isGridModeEnabled(this.node) && !this.retakeMode;
    if (this.autoGridBtn) this.autoGridBtn.style.display = enabled ? "" : "none";
    if (this.syncGridTextBtn) this.syncGridTextBtn.style.display = enabled ? "" : "none";
    for (const w of [this.gridTextWidget, this.gridLayoutWidget, this.gridAspectWidget, this.gridBorderCropWidget, this.gridAutoRefreshWidget]) {
      if (!w) continue;
      if (enabled) showWidget(w);
      else hideWidget(w);
    }
    this.node?.setDirtyCanvas?.(true, true);
  }
  ...
  getCurrentGridText() {
    const origin = getOriginNode(this.node, "grid_text");
    if (origin?.widgets?.length) {
      const value = origin.widgets[0]?.value;
      if (value !== undefined && value !== null && String(value).trim()) return String(value);
    }
    return String(this.gridTextWidget?.value || "");
  }

  currentGridInputKey(source = null, sourceKey = "") {
    source = source || getGridSource(this.node);
    sourceKey = sourceKey || gridSourceKey(source);
    return sourceKey || "";
  }

  async ensureGridPreviewImage(source = null, sourceKey = "") {
    source = source || getGridSource(this.node);
    sourceKey = sourceKey || gridSourceKey(source);
    if (!source?.url || !sourceKey) return null;
    if (this._gridPreviewImage && this._gridPreviewKey === sourceKey) return this._gridPreviewImage;
    if (this._gridPreviewLoad && this._gridPreviewKey === sourceKey) return this._gridPreviewLoad;

    this._gridPreviewKey = sourceKey;
    this._gridPreviewImage = null;
    this._gridPreviewLoad = imageFromUrl(source.url)
      .then(img => {
        if (this._gridPreviewKey === sourceKey) {
          this._gridPreviewImage = img;
          this.render();
        }
        return img;
      })
      ...
      .finally(() => {
        if (this._gridPreviewKey === sourceKey) this._gridPreviewLoad = null;
      });
    return this._gridPreviewLoad;
  }

  checkGridSourceChange() {
    if (!isGridModeEnabled(this.node)) return;
    if (this.gridAutoRefreshWidget && this.gridAutoRefreshWidget.value === false) return;
    const now = performance.now ? performance.now() : Date.now();
    if (now - this._lastGridCheck < 1000) return;
    this._lastGridCheck = now;
    const source = getGridSource(this.node);
    const sourceKey = gridSourceKey(source);
    if (!sourceKey) return;
    this.ensureGridPreviewImage(source, sourceKey);
    this.requestGridAutoRefresh("source", source, sourceKey);
  }

  requestGridAutoRefresh(reason = "source", source = null, sourceKey = "") {
    if (!isGridModeEnabled(this.node)) return false;
    source = source || getGridSource(this.node);
    sourceKey = sourceKey || gridSourceKey(source);
    if (!sourceKey) return false;

    const inputKey = this.currentGridInputKey(source, sourceKey);
    const existingGridKey = (this.timeline.segments || []).find(s => s.source === "grid_images")?.gridSourceKey || "";
    if (!inputKey || inputKey === this._lastGridInputKey || inputKey === existingGridKey) return false;
    this._lastGridInputKey = inputKey;
    this._lastGridSourceKey = sourceKey;

    if (this._gridAutoRefreshTimer) clearTimeout(this._gridAutoRefreshTimer);
    this._gridAutoRefreshTimer = setTimeout(() => {
      this._gridAutoRefreshTimer = null;
      this.autoFillFromGrid(false, source, sourceKey);
    }, 120);
    return true;
  }

  cropGridPreview(img, idx, cols, rows) {
    const col = idx % cols;
    const row = Math.floor(idx / cols);
    const sx0 = Math.round(col * img.naturalWidth / cols);
    const sx1 = Math.round((col + 1) * img.naturalWidth / cols);
    const sy0 = Math.round(row * img.naturalHeight / rows);
    const sy1 = Math.round((row + 1) * img.naturalHeight / rows);
    let sw = Math.max(1, sx1 - sx0);
    let sh = Math.max(1, sy1 - sy0);
    let sx = sx0;
    let sy = sy0;

    const strength = Math.max(0, Number(this.gridBorderCropWidget?.value ?? 1.0) || 0);
    const padX = Math.round(sw * 0.012 * strength);
    const padY = Math.round(sh * 0.012 * strength);
    if (sw > padX * 2 + 8) {
      sx += padX;
      sw -= padX * 2;
    }
    ...
    const aspectText = String(this.gridAspectWidget?.value || "");
    let targetAspect = null;
    if (aspectText.includes("16:9")) targetAspect = 16 / 9;
    else if (aspectText.includes("9:16")) targetAspect = 9 / 16;
    else if (aspectText.includes("1:1") || aspectText.includes("方")) targetAspect = 1;
    if (targetAspect) {
      const current = sw / sh;
      if (current > targetAspect) {
        const newW = Math.round(sh * targetAspect);
        sx += Math.max(0, Math.floor((sw - newW) / 2));
        sw = newW;
      } else {
        const newH = Math.round(sw / targetAspect);
        sy += Math.max(0, Math.floor((sh - newH) / 2));
        sh = newH;
      }
    }
    ...
    const out = new Image();
    out.src = dataUrl;
    return { dataUrl, imgObj: out };
  }

  async autoFillFromGrid(onlyIfEmpty = false, source = null, sourceKey = "") {
    if (!isGridModeEnabled(this.node)) return false;
    source = source || getGridSource(this.node);
    sourceKey = sourceKey || gridSourceKey(source);
    if (!source?.url || !sourceKey) return false;
    if (onlyIfEmpty && this.timeline.segments.length > 0) return false;

    const img = await this.ensureGridPreviewImage(source, sourceKey);
    if (!img) return false;

    const count = source.cols * source.rows;
    const lengths = evenGridLengths(this.getDurationFrames(), count);
    const prompts = parseGridPrompts(this.getCurrentGridText());
    let cursor = 0;
    const segments = [];
    for (let idx = 0; idx < count; idx++) {
      const preview = this.cropGridPreview(img, idx, source.cols, source.rows);
      ...
      segments.push({
        ...
        grid_index: idx,
        batch_index: idx,
        gridSourceKey: sourceKey,
        guideStrength: 1.0,
        imageB64: preview.dataUrl,
        imgObj: preview.imgObj,
      });
      cursor += length;
    }
    this.timeline.segments = segments;
    this.selectionType = "image";
    this.selectedIndex = segments.length ? 0 : -1;
    this.updateUIFromSelection();
    this.commitChanges(true);
    this.render();
    return true;
  ...
  syncGridPromptsFromText() {
    if (!isGridModeEnabled(this.node)) return false;
    const prompts = parseGridPrompts(this.getCurrentGridText());
    if (!prompts.length) return false;
    let idx = 0;
    for (const seg of this.timeline.segments || []) {
      if (seg.source !== "grid_images") continue;
      if (idx < prompts.length) seg.prompt = prompts[idx];
      idx++;
    }
    this.updateUIFromSelection();
    this.commitChanges(true);
  ...
  // own /cs_ltx_director_upload_chunk endpoint to bypass the 413 size limit.
  async _uploadVideoFile(file) {
    ...
    try {
      ...
      if (checkResp.status === 200) {
        ...
        if (checkResult.exists) {
          console.log(`[CSLTXDirector] File already exists: ${checkResult.name}. Reusing existing file.`);
          return checkResult.name;
        }
      }
    } catch (e) {
      console.warn("[CSLTXDirector] Failed to check for existing file, proceeding with upload", e);
    }
    ...
    if (file.size > CHUNK_SIZE) {
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        const resp = await api.fetchApi("/cs_ltx_director_upload_chunk", { method: "POST", body: formData });
        ...
        try {
          ...
          vid.onloadeddata = async () => {
            ...
            vid.onseeked = () => {
              ...
              if (IS_LARGE_FILE) {
                console.log(`[CSLTXDirector] Large file detected (${(file.size / 1024 / 1024).toFixed(1)} MB). Offloading audio extraction to server.`);
              } else {
              ...
              this._uploadVideoFile(file).then(filePath => {
                ...
                if (filePath) {
                  api.fetchApi(`/cs_ltx_director_get_audio?filename=${encodeURIComponent(filePath)}`)
                    .then(r => r.json())
                    .then(res => {
                      if (res.audio_file && res.peaks) {
                        ...
                        // Fallback
                        if (IS_LARGE_FILE) {
                          console.warn("[CSLTXDirector] Server audio extraction failed for large file, skipping.");
                          for (let s of this.timeline.audioSegments) {
                            if (s.audioFile === filePath || s._blobUrl === blobUrl) {
                              s._decoding = false;
                            }
                    ...
                    .catch(err => {
                      console.error("[CSLTXDirector] Server audio extraction query failed:", err);
                      for (let s of this.timeline.audioSegments) {
                        if (s.audioFile === filePath || s._blobUrl === blobUrl) {
                          s._decoding = false;
                        }
                ...
                console.error("[CSLTXDirector] Background video upload failed", err);
    ...
    for (let file of files) {
      ...
      await new Promise(async (resolve) => {
        try {
          ...
          vid.onloadeddata = () => {
            ...
            vid.onseeked = () => {
              ...
              this._uploadVideoFile(file).then(filePath => {
                ...
                console.error("[CSLTXDirector] Background motion video upload failed", err);
          ...
          console.error("[CSLTXDirector] Motion video processing failed", err);
          resolve();
        }
      });
    }
  ...
  updateUIFromSelection() {
    if (this.selectedSegmentIds && this.isMultiSelectActive()) {
      ...
      if (this.promptInput) {
        ...
        this.promptInput.placeholder = "已选择多个分段";
      ...
      if (this.strengthValue) {
        ...
        this.strengthValue.placeholder = "多个";
        this.strengthValue.disabled = true;
      ...
      if (this.segmentBoundsDisplay) {
        this.segmentBoundsDisplay.textContent = "已选择多个分段";
      }
      return;
    }
    ...
    if (this.retakeMode) {
      ...
      this.promptInput.placeholder = "输入 Retake 区间提示词...";
      ...
      if (this.segmentBoundsDisplay) {
        ...
        this.segmentBoundsDisplay.textContent = `开始：${startStr} | 结束：${endStr} | 长度：${lengthStr}`;
      }
      ...
      this.audioInfoArea.innerHTML = `
        文件：<span>${seg.fileName || "未知"}</span><br>
        原始长度：<span>${this.formatTime(seg.audioDurationFrames)}</span> 输出长度：<span>${this.formatTime(seg.length)}</span><br>
        裁入：<span>${this.formatTime(Math.round(seg.trimStart))}</span> 裁出：<span>${this.formatTime(Math.round(seg.audioDurationFrames - (seg.trimStart + seg.length)))}</span>
      `;
      this.strengthValue.value = "1.00";
      this.strengthValue.disabled = true;
      ...
      this.promptInput.disabled = false;
      this.promptInput.style.opacity = "1.0";
      this.promptInput.placeholder = "全局提示词（同步到所有 IC-LoRA 视频分段）...";
      ...
      if (seg) {
        if (this.selectionType !== "motion") {
          ...
          this.promptInput.placeholder = "输入当前分段提示词...";
        }
        this.promptInput.disabled = false;
        this.promptInput.style.opacity = "1.0";
  ...
  updateRetakeUIState() {
    ...
    if (isRetake) {
      ...
      if (this.uploadVideoBtn) {
        this.uploadVideoBtn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg> 添加视频`;
      }
      if (this.mainTrackLabel) {
        const textSpan = this.mainTrackLabel.querySelector("span");
    ...
    if (this.segmentPromptLabel) {
      this.segmentPromptLabel.textContent = isRetake ? "Retake 提示词" : "局部提示词";
    }
    ...
    if (this.retakeMode) {
      ...
      if (retakeVid) {
        ...
        if (retakeVid._uploading || retakeVid._extractingThumbs) {
          ...
          const upText = retakeVid._extractingThumbs ? "正在提取帧..." : "正在上传基础视频...";
        ...
        this.ctx.fillText("拖拽或点击添加视频", activeStart + activeWidth / 2, RULER_HEIGHT + this.blockHeight / 2);
      }
        ...
        if ((seg.type === "video" || drawSource) && seg.type !== "ghost") {
          if (seg.type === "video" && pxWidth > 0) {
            ...
            if ((seg._uploading || seg._extractingThumbs) && pxWidth > 60) {
              ...
              const upText = seg._extractingThumbs ? "加载中..." : "上传中...";
      ...
      for (let i = 0; i < sortedMotionSegments.length; i++) {
        ...
        if (seg.type === "ghost") {
          ...
          if (pxWidth > 0 && seg.type !== "ghost") {
            ...
            this.ctx.textAlign = "center";
            this.ctx.textBaseline = "middle";
            this.ctx.fillText("IC 视频", startX + 37, trackY + 9);
            this.ctx.restore();
  ...
  drawAudioSegmentVisuals(ctx, seg, isSelected, yOffset, trackHeight, startX, pxWidth, outlineColor = null, showHandles = true) {
    ...
    if ((seg._uploading || seg._decoding) && pxWidth > 60) {
      ...
      const upText = seg._decoding ? "解码中..." : "上传中...";
  ...
  commitChanges(skipRender = false) {
    ...
    console.log("[CSLTXDirector debug] commitChanges: saving timelineDataWidget value:", jsonStr);
  ...
  showContextMenu(clientX, clientY, seg, trackType) {
    ...
    if (isImage) {
      ...
      replaceImgBtn.innerHTML = `替换为剪贴板图片`;
    ...
    markSelectionBtn.innerHTML = `标记选区`;
  ...
  showGapContextMenu(clientX, clientY, gap) {
    ...
    if (currentTrack === "image") {
      ...
      textBtn.innerHTML = `${ICONS.text} 文字分段`;
      textBtn.onclick = () => {
      ...
      imgBtn.innerHTML = `${ICONS.upload} 图片分段`;
      imgBtn.onclick = () => {
        this.dismissContextMenu();
        const fi = document.createElement("input");
      ...
      vidBtn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg> 视频分段`;
      vidBtn.onclick = () => {
        this.dismissContextMenu();
        const fi = document.createElement("input");
      ...
      audBtn.innerHTML = `${ICONS.audio} 音频分段`;
      audBtn.onclick = () => {
        this.dismissContextMenu();
        this.promptAddAudioInGap(gap.frameStart, gap.frameEnd);
      };
  ...
  showSettingsMenu(anchorEl) {
    ...
    toggleBtn.textContent = widgetsVisible ? "隐藏参数" : "显示参数";
    toggleBtn.addEventListener("click", () => {
      ...
      if (nowVisible) {
        ...
        toggleBtn.textContent = stillVisible ? "隐藏参数" : "显示参数";
      } else {
        this.showSettingsWidgets();
        toggleBtn.textContent = "隐藏参数";
      }
    });
    ...
    if (dmWidget) {
      ...
      menu.appendChild(this._makeSettingRow("时间显示方式", ctrl));
    }
    ...
    if (epsWidget) {
      menu.appendChild(this._makeSettingRow("分段边界强度", createScrubbableNumberControl(epsWidget, 0.0001, 0.0001, 0.99, true)));
    }
    ...
    btnOpenFolder.addEventListener("click", async () => {
      try {
        const response = await api.fetchApi("/cs_ltx_director_open_folder");
        const data = await response.json();
        if (!data.success) {
          console.error("Failed to open workspace folder:", data.error || "Unknown error");
          alert("无法打开工作目录。这个功能只支持本地运行的 ComfyUI。");
        }
      } catch (err) {
        console.error("Error opening workspace folder:", err);
        alert("打开工作目录出错：" + err.message);
      }
    });

    menu.appendChild(this._makeSettingRow("工作目录", btnOpenFolder));
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "CSLTXDirector") {
      ...
      nodeType.prototype.onNodeCreated = function () {
        ...
        const DEFAULTS = {
          ...
          grid_mode: false,
          grid_layout: "3x2 六宫格",
          grid_aspect: "自动 / 保持单格比例",
          grid_border_crop: 1.0,
          grid_auto_refresh: true,
          grid_text: "",
        };
        ...
        applyChineseDirectorLabels(this);
        const isLiteGraph = !window.LiteGraph || !window.LiteGraph.vueNodesMode;
      ...
      nodeType.prototype.onConfigure = function (info) {
        ...
        console.log("[CSLTXDirector debug] onConfigure called. info.widgets_values:", info.widgets_values ? JSON.stringify(info.widgets_values) : "undefined");
        ...
        if (info.properties && info.properties.has_serialized_properties) {
          console.log("[CSLTXDirector debug] Restoring widgets from properties");
          if (this.widgets) {
            for (const w of this.widgets) {
          ...
          console.log("[CSLTXDirector debug] Restoring widgets via fallback name-based schema mapping");
          const SCHEMA_19 = [
          ...
          const SCHEMA_GRID = [
            ...
            "timeline_data", "use_custom_audio", "use_custom_motion", "inpaint_audio", "local_prompts", "segment_lengths",
            "epsilon", "frame_rate", "display_mode", "guide_strength", "custom_width", "custom_height",
            "resize_method", "divisible_by", "img_compression", "override_audio",
            "grid_mode", "grid_layout", "grid_aspect", "grid_border_crop", "grid_auto_refresh", "timeline_ui"
          ];
          const SCHEMA_GRID_WITH_TEXT_WIDGET = [
            "start_second", "end_second", "duration_seconds", "start_frame", "end_frame", "duration_frames",
            "timeline_data", "use_custom_audio", "use_custom_motion", "inpaint_audio", "local_prompts", "segment_lengths",
            "epsilon", "frame_rate", "display_mode", "guide_strength", "custom_width", "custom_height",
            "resize_method", "divisible_by", "img_compression", "override_audio",
            "grid_mode", "grid_text", "grid_layout", "grid_aspect", "grid_border_crop", "grid_auto_refresh", "timeline_ui"
          ];
          ...
          } else if (len >= SCHEMA_GRID.length) {
            names = len >= SCHEMA_GRID_WITH_TEXT_WIDGET.length ? SCHEMA_GRID_WITH_TEXT_WIDGET : SCHEMA_GRID;
          }
        ...
        setTimeout(() => {
          if (this._timelineEditor) {
            console.log("[CSLTXDirector debug] setTimeout sync block called.");
            console.log("[CSLTXDirector debug] setTimeout: timelineDataWidget value:", this._timelineEditor.timelineDataWidget?.value);
            const tl = parseInitial(this._timelineEditor.timelineDataWidget?.value);
            console.log("[CSLTXDirector debug] setTimeout: parsed timeline:", JSON.stringify(tl));
